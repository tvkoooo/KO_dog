...
//////////////////////////////////////////////////////////////////////////
void mm_business_relation_tcp_hd_c_business_relation_allow_friend_rq(void* obj, void* u, struct mm_packet* rq_pack)
{
	c_business_relation::allow_friend_rq rq_msg;
...
//	struct mm_business_nt_message_sql* nt_message_sql= &impl->nt_message_sql;
	...
	do 
	{
		...
		//rs包部分数据更新
		rs_msg.set_user_allow_id(user_allow_id);
		rs_msg.set_opcode(opcode);
		//////////////////////////////////////////////////////////////////////////
		//拼装nt包部分数据
		nt_msg.set_opcode(opcode);
		...
		//无论opcode 是多少，说明处理了好友申请，需要删除好友申请表里面别人发给自己的记录
		//操作数据库（p_user_relation_respond_apply）
		struct mm_db_mysql* db_mysql = mm_db_mysql_section_thread_instance(&impl->db_sql_section);
		{

			mm::p_user_relation_respond_apply query;
			query.user_allow_id = user_myself_id;//respond 是自己id
			query.user_apply_id = user_allow_id;//这个user_allow_id是自己同意对象，申请人id
		...
		if (1 == opcode)
		{
			struct mm_db_mysql* db_mysql = mm_db_mysql_section_thread_instance(&impl->db_sql_section);
			{
				...
				//rs包的relation_s   rs_msg.mutable_relation_s();
				{
					...
					//拼装sql返回select容器内容
					//typedef 做一个短名称
					typedef mm::p_user_relation_add_friend::user_relation_vec user_relation_vec;
				...
				//nt包的relation_s   nt_msg.mutable_relation_s();
				{
					b_user_relation_v* user_relation_s = nt_msg.mutable_relation_s();

					mm::p_user_relation_add_friend query;
					query.myself_id = user_allow_id;
					query.friend_id = user_myself_id;
					query.friend_remark = user_myself_nick;
					if (db_mysql_state_success != query.query(db_mysql, MM_LOG_INFO))
					{
						error_info->set_code(err_common_exec_procedure_failure);
						error_info->set_desc(mm_error_desc_string(error_desc, error_info->code()));
		...
		//	user_myself_id, user_allow_id, c_business_relation::allow_friend_nt_msg_id, &nt_msg,
		//	&nt_pack);
		//////////////////////////////////////////////////////////////////////////
		//对nt包进行 cback 回推发送
		struct mm_packet_head packet_head;

		packet_head.mid = c_business_relation::allow_friend_nt_msg_id;
		packet_head.pid = 0;
		packet_head.sid = 0;
		packet_head.uid = user_allow_id;
	...
	// rs
	mm_tcp_o_lock(tcp);
	mm_protobuf_cxx_n_tcp_append_rs(mailbox, tcp, c_business_relation::allow_friend_rs_msg_id, &rs_msg, rq_pack, &rs_pack);
