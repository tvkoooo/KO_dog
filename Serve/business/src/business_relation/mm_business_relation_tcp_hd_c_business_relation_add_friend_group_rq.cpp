...
//////////////////////////////////////////////////////////////////////////
void mm_business_relation_tcp_hd_c_business_relation_add_friend_group_rq(void* obj, void* u, struct mm_packet* rq_pack)
{
	c_business_relation::add_friend_group_rq rq_msg;
	...
	do 
	{
		...
		struct mm_db_mysql* db_mysql = mm_db_mysql_section_thread_instance(&impl->db_sql_section);
		{
			...
			//rs更新执行sql后的返回部分数据
			rs_msg.set_user_friend_group_id(query.id);
		}
	} while (0);
	...
	// rs
	mm_tcp_o_lock(tcp);
	mm_protobuf_cxx_n_tcp_append_rs(mailbox, tcp, c_business_relation::add_friend_group_rs_msg_id, &rs_msg, rq_pack, &rs_pack);
