...
void mm_application_init(struct mm_application* p)
{
	...
	p->state = 0;
	mm_business_relation_init(&p->impl);
	//
...
void mm_application_destroy(struct mm_application* p)
{
	...
	mm_business_relation_destroy(&p->impl);
	mm_logger_log_I(g_logger,"%s %d.",__FUNCTION__,__LINE__);
...
void mm_application_initialize(struct mm_application* p,int argc,const char **argv)
{
	...
	if (p->argument.argc == g_argument.argc)
	{
		...
		mm_string_assigns(&log_name, "mm_business_relation_");
		...
		////////////////////////////////////////////////////////
		mm_business_relation_assign_unique_id(&p->impl,mm_atoi32(p->argument.argv[3]));
		mm_business_relation_assign_internal_mailbox_parameters(&p->impl,p->argument.argv[4]);
		mm_business_relation_assign_external_mailbox_parameters(&p->impl,p->argument.argv[5]);
		mm_business_relation_assign_zookeeper_export_parameters(&p->impl, p->argument.argv[6]);
		mm_business_relation_assign_module_number(&p->impl, mm_atoi32(p->argument.argv[7]));
		mm_business_relation_assign_area_shard(&p->impl, mm_atoi32(p->argument.argv[8]));
		mm_business_relation_assign_area_depth(&p->impl, mm_atoi32(p->argument.argv[9]));
		////////////////////////////////////////////////////////
		p->state = 1;
	}
	else
...
void mm_application_start(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		...
		//
		mm_business_relation_start(&p->impl);
		//
...
void mm_application_interrupt(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_business_relation_interrupt(&p->impl);
...
void mm_application_shutdown(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_business_relation_shutdown(&p->impl);
...
void mm_application_join(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		struct mm_context* g_context = mm_context_instance();
		struct mm_time_cache* g_time_cache = mm_time_cache_instance();
		mm_business_relation_join(&p->impl);
