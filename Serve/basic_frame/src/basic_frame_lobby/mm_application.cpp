...
{
	mm_printf("%s\n", "help:");
	mm_printf("%s\n", "程序名             日志目录 日志等级 实例编号  内地址启动参数   外地址启动参数   读取监控集群号   写入监控集群号   账号缓存地址号        网关缓存地址号       ");
	mm_printf("%s\n", "program_name       log_dir  log_lvl  unique_id internal_mailbox external_mailbox zookeeper_import zookeeper_export rdb_account_cache     rdb_gateway_cache    ");
	mm_printf("%s\n", "./mm_basic_frame_lobby ./log    7        1         ::-65535[2]      ::-65534[2]      127.0.0.1:10300, 127.0.0.1:10300, 192.168.111.123-65535 192.168.111.123-65535");
}
/////////////////////////////////////////////////////////////////
...
void mm_application_init(struct mm_application* p)
{
	...
	p->state = 0;
	mm_basic_frame_lobby_init(&p->impl);
	//
...
void mm_application_destroy(struct mm_application* p)
{
	...
	mm_basic_frame_lobby_destroy(&p->impl);
	mm_logger_log_I(g_logger,"%s %d.",__FUNCTION__,__LINE__);
...
void mm_application_initialize(struct mm_application* p,int argc,const char **argv)
{
	...
	if (p->argument.argc == g_argument.argc)
	{
		...
		mm_string_assigns(&g_logger_manager->file_name,"mm_basic_frame_lobby");
		...
		////////////////////////////////////////////////////////
		mm_basic_frame_lobby_assign_unique_id(&p->impl,mm_atoi32(p->argument.argv[3]));
		mm_basic_frame_lobby_assign_internal_mailbox_parameters(&p->impl,p->argument.argv[4]);
		mm_basic_frame_lobby_assign_external_mailbox_parameters(&p->impl,p->argument.argv[5]);
		mm_basic_frame_lobby_assign_zookeeper_import_parameters(&p->impl,p->argument.argv[6]);
		mm_basic_frame_lobby_assign_zookeeper_export_parameters(&p->impl,p->argument.argv[7]);
		mm_basic_frame_lobby_assign_rdb_account_cache_parameters(&p->impl,p->argument.argv[8]);
		mm_basic_frame_lobby_assign_rdb_gateway_cache_parameters(&p->impl,p->argument.argv[9]);
		////////////////////////////////////////////////////////
		p->state = 1;
	}
	else
...
void mm_application_start(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		...
		//
		mm_basic_frame_lobby_start(&p->impl);
		//
...
void mm_application_interrupt(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_basic_frame_lobby_interrupt(&p->impl);
...
void mm_application_shutdown(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_basic_frame_lobby_shutdown(&p->impl);
...
void mm_application_join(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		struct mm_context* g_context = mm_context_instance();
		struct mm_time_cache* g_time_cache = mm_time_cache_instance();
		mm_basic_frame_lobby_join(&p->impl);
