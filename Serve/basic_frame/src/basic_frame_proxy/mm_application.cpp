...
void mm_application_init(struct mm_application* p)
{
	...
	p->state = 0;
	mm_basic_frame_proxy_init(&p->impl);
	//
...
void mm_application_destroy(struct mm_application* p)
{
	...
	mm_basic_frame_proxy_destroy(&p->impl);
	mm_logger_log_I(g_logger,"%s %d.",__FUNCTION__,__LINE__);
...
void mm_application_initialize(struct mm_application* p,int argc,const char **argv)
{
	...
	if (p->argument.argc == g_argument.argc)
	{
		...
		mm_string_assigns(&g_logger_manager->file_name,"mm_basic_frame_proxy");
		...
		////////////////////////////////////////////////////////
		mm_basic_frame_proxy_assign_unique_id(&p->impl,mm_atoi32(p->argument.argv[3]));
		mm_basic_frame_proxy_assign_server_id(&p->impl,mm_atoi32(p->argument.argv[4]));
		mm_basic_frame_proxy_assign_internal_mailbox_parameters(&p->impl,p->argument.argv[5]);
		mm_basic_frame_proxy_assign_module(&p->impl,mm_atoi32(p->argument.argv[6]));
		mm_basic_frame_proxy_assign_message_interval(&p->impl,p->argument.argv[7],p->argument.argv[8]);
		mm_basic_frame_proxy_assign_shard(&p->impl,mm_atoi32(p->argument.argv[9]));
		////////////////////////////////////////////////////////
		p->state = 1;
	}
	else
...
void mm_application_start(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		...
		//
		mm_basic_frame_proxy_start(&p->impl);
		//
...
void mm_application_interrupt(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_basic_frame_proxy_interrupt(&p->impl);
...
void mm_application_shutdown(struct mm_application* p)
{
	...
	struct mm_context* g_context = mm_context_instance();
	struct mm_time_cache* g_time_cache = mm_time_cache_instance();
	mm_basic_frame_proxy_shutdown(&p->impl);
...
void mm_application_join(struct mm_application* p)
{
	...
	if ( 1 == p->state )
	{
		struct mm_context* g_context = mm_context_instance();
		struct mm_time_cache* g_time_cache = mm_time_cache_instance();
		mm_basic_frame_proxy_join(&p->impl);
